package com.hrms.service;

import com.hrms.entity.AssetCategory;
import com.hrms.repository.AssetCategoryRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

/**
 * AssetCategory Service
 * Module: ASSETS
 */
@Service
@Transactional
public class AssetCategoryService {

    @Autowired
    private AssetCategoryRepository repository;

    /**
     * Get all records by organization with pagination
     */
    public Page<AssetCategory> getByOrganization(UUID organizationId, Pageable pageable) {
        return repository.findByOrganizationId(organizationId, pageable);
    }

    /**
     * Get all records by organization
     */
    public List<AssetCategory> getAllByOrganization(UUID organizationId) {
        return repository.findByOrganizationId(organizationId);
    }

    /**
     * Get record by ID
     */
    public Optional<AssetCategory> getById(UUID id) {
        return repository.findById(id);
    }

    /**
     * Create new record
     */
    public AssetCategory create(AssetCategory entity) {
        return repository.save(entity);
    }

    /**
     * Update existing record
     */
    public AssetCategory update(UUID id, AssetCategory updatedEntity) {
        return repository.findById(id)
            .map(existing -> {
                updatedEntity.setId(id);
                return repository.save(updatedEntity);
            })
            .orElseThrow(() -> new RuntimeException("AssetCategory not found with id: " + id));
    }

    /**
     * Delete record
     */
    public void delete(UUID id) {
        repository.deleteById(id);
    }

    /**
     * Count records by organization
     */
    public Long countByOrganization(UUID organizationId) {
        return repository.countByOrganizationId(organizationId);
    }
}
