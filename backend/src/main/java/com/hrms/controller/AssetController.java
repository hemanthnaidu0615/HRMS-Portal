 ...
 */
@RestController
@RequestMapping("/api/assets/asset")
...
public class AssetController {
    ...
    public ResponseEntity<Page<Asset>> getAll(
            ...
            Pageable pageable) {
        try {
            Page<Asset> records = service.getByOrganization(organizationId, pageable);
     ...
     */
    @GetMapping("/all")
    public ResponseEntity<List<Asset>> getAllNoPagination(@RequestParam UUID organizationId) {
        try {
            List<Asset> records = service.getAllByOrganization(organizationId);
            return ResponseEntity.ok(records);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    ...
    public ResponseEntity<Asset> create(@RequestBody Asset entity) {
        try {
            Asset created = service.create(entity);
    ...
    public ResponseEntity<Asset> update(
            ...
            @RequestBody Asset entity) {
        try {
            Asset updated = service.update(id, entity);
