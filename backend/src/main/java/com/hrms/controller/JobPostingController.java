...
public class JobPostingController {
     ...
     */
    @GetMapping
    public ResponseEntity<Page<JobPosting>> getAll(
            @RequestParam UUID organizationId,
            Pageable pageable) {
        try {
            Page<JobPosting> records = service.getByOrganization(organizationId, pageable);
     ...
     */
    @GetMapping("/all")
    public ResponseEntity<List<JobPosting>> getAllNoPagination(@RequestParam UUID organizationId) {
        try {
            List<JobPosting> records = service.getAllByOrganization(organizationId);
            return ResponseEntity.ok(records);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
     ...
     */
    @PostMapping
    public ResponseEntity<JobPosting> create(@RequestBody JobPosting entity) {
        try {
            JobPosting created = service.create(entity);
    ...
    public ResponseEntity<JobPosting> update(
            ...
            @RequestBody JobPosting entity) {
        try {
            JobPosting updated = service.update(id, entity);
