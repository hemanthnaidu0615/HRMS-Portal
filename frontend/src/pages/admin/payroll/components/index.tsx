...
const ComponentsListPage: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    Modal.confirm({
      title: 'Delete Salary Components',
      content: 'Are you sure you want to delete this salary components?',
      onOk: async () => {
        try {
          const token = localStorage.getItem('token');
          await axios.delete(
          ...
          message.success('Salary Components deleted successfully');
          fetchData();
        } catch (error: any) {
          message.error(error.response?.data?.message || 'Failed to delete salary components');
        }
      },
    });
  };
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            type="link"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/admin/payroll/components/${record.id}`)}
          >
            View
          </Button>
          <Button
