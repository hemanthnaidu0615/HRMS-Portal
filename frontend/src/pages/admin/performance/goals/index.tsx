...
const GoalsListPage: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    Modal.confirm({
      title: 'Delete Goals',
      content: 'Are you sure you want to delete this goals?',
      onOk: async () => {
        try {
          const token = localStorage.getItem('token');
          await axios.delete(
          ...
          message.error(error.response?.data?.message || 'Failed to delete goals');
        }
      },
    });
  };
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            type="link"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/admin/performance/goals/${record.id}`)}
          >
            View
          </Button>
          <Button
            type="link"
            icon={<EditOutlined />}
            onClick={() => navigate(`/admin/performance/goals/${record.id}/edit`)}
          >
            Edit
          </Button>
          <Button
