...
const ReviewsListPage: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    Modal.confirm({
      ...
      content: 'Are you sure you want to delete this reviews?',
      onOk: async () => {
        try {
          const token = localStorage.getItem('token');
          await axios.delete(
          ...
          message.error(error.response?.data?.message || 'Failed to delete reviews');
        }
      },
    });
  };
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            type="link"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/admin/performance/reviews/${record.id}`)}
          >
            View
          </Button>
          <Button
            type="link"
            icon={<EditOutlined />}
            onClick={() => navigate(`/admin/performance/reviews/${record.id}/edit`)}
          >
            Edit
          </Button>
          <Button
